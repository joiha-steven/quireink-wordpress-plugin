...
function quireink_pen_warning( $post, $state ) {
	...
	if ( 'foreign' === $state ) {
		$lead = __( 'This post has been edited in WordPress since Quire Ink last saved it. What is below is the WordPress version, which is the newer one; the older Markdown source has been set aside.', 'quire-ink-pen' );
	} elseif ( $at_risk ) {
		$lead = sprintf(
		...
		$lead = __( 'This post was not written in Quire Ink. Its HTML has been brought in as it stands; from the first save on, the Markdown is the source.', 'quire-ink-pen' );
	} else {
		return;
	}

	?>
	<div class="<?php echo esc_attr( 'quireink-pen-warning notice notice-warning inline' . ( $at_risk ? ' quireink-pen-warning-loud' : '' ) ); ?>">
		<?php if ( $at_risk ) : ?>
			<p><strong><?php esc_html_e( 'Saving here will replace this post\'s layout.', 'quire-ink-pen' ); ?></strong></p>
		<?php endif; ?>
